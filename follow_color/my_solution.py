...
def execute(img):
    center_image_x = 320 / 2
    center_image_y = 240 / 2
    target_radius = 10
    ...
    if len(contours) > 0:
        ...
        if mom["m00"] > 0:
            ...
            cv2.arrowedLine(tmp_img, (int(x), int(y)), (center_image_x + y_error, center_image_y),
                            (255, 0, 0), thickness=3)
    ...
    vz = p_controller(0.05, z_error) + d_controller_2(0.005, z_error)
    vx = -p_controller(0.2, x_error) + d_controller_3(0.0002, x_error)
    return -vy, vx, vz, 0
...
def main():
    ...
    drone.takeoff(h=5, precision=0.2)
    print("Taken off")
